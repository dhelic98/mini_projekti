	...
	public static void main(String[] args) {
		...
		do {
			try {
				...
				long cardNumber = uInput.nextLong();
				// card number must be positive
				while (cardNumber <= 0) {
					System.out.println("Input must be positive");
					cardNumber = uInput.nextLong();
				}

				// condition for card number length
				while ((getSize(cardNumber) < 13) || (getSize(cardNumber) > 16)) {
					System.out
							.println("Credit card number must be between 13 and 16 digits long");
					System.out.println("Input again");
					cardNumber = uInput.nextLong();

				}
				// Printing out whether the card number is valid
